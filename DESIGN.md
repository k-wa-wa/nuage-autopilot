...
レビューを済ませた PR が上がってくる。CI が緑になった PR は verify（8.4 節）が preview 環境で
検証し、問題があればエージェント自身に差し戻す。人間が明示的に行うのは「書くこと」と
「マージすること」だけである。
...
5. （本システムの外）PR に基づいて preview 環境が自動で立ち上がる
6. verify が preview を検証し、問題があれば PR に指摘を書く
7. エージェントが裏で動き、PR を修正する（6 に戻る）
8. 人間がマージする

Issue が大きい場合は、エージェントがサブ Issue に分割しながら進める。

**人間の行動が起点になるのは 1・3・8 だけである。** 6 と 7 の往復は機械の中で閉じており、
人間がコメントすればいつでもそこに割り込める（`commented` は phase を問わずエージェントを起こす）。

5 は本システムの実装対象ではない。対象リポジトリ側（CI と Argo CD）で用意される前提とし、
verify はそれを利用する。
...
| `in_review` | verify_failure | エージェント起動（resume）。verify の差し戻し（8.4 節） |
...
| `verify_failed` | 検証を実施し明確な不合格を確認した | `in_review` のまま（差し戻し。8.4 節） |
...
`ready` は「CI が緑で、人間のマージ待ち」の状態である。そこで人間が供給しているものは
**検証**（要求どおり動いているか）と**リスク受容**（壊れても戻せるか）の 2 つしかない。
verify は前者を機械に移す。後者（可逆性・自動ロールバック）は本システムの範囲外であり、
**verify が通っただけでは「マージしてよい」と結論してはならない**。

verify は常に走る。無効化スイッチは持たない。判定を出せない状況では PR を止めずに
素通りさせるため（後述）、スイッチを設けるより状態が少なく済む。

**verify は gate であると同時にフィードバック源である。** 合否だけを返す verify は不合格の
PR を溜めるだけで人間の関与を減らさない。そのため不合格時は phase を止めず、エージェントを
起こし直すイベント `verify_failure` を engine が積む。これにより「1 イベント = 高々 1 回の
LLM 起動」が保たれ、予算も起動ごとに正しく積算される。差し戻し理由は PR コメント
（人間チャネル）に置き、イベントはその所在を指すだけに留める（8.2 節）。

**verify はコードを変更しない。** 書き込み先は PR コメント 1 件と `NUAGE_REPORT_FILE` だけとし、
commit / push / approve / merge を禁じる。同じ理由で実装エージェントのセッションを
`--resume` せず、`session_id` も保存しない。修正権限や思考過程を引き継いだ verify は
第三者ではなく実装エージェントそのものであり、判定に意味が無くなるためである。

#### verify は動作確認とコードレビューの両方を担う

CI は決定的に判定できるもの（ビルド・テスト・lint）しか見ない。要求を満たしているか、
規約から逸脱していないか、見落とされた欠陥が無いかは判定できず、**マージ前に機械が
品質を見る機会は verify しか無い**。したがって verify は動作確認に加えてコードレビューも担う。
実装エージェントの自己レビューでは代替できない。同じセッションは同じ盲点を持つためであり、
verify を別セッションにしたのと同じ理由である。

ただし **差し戻せるのは客観的に示せるものに限る**。要求の未充足、動作不良、`AGENTS.md` 等に
明文化された規約への違反、バグ・セキュリティ・データ破壊につながる欠陥がそれにあたる。
好みや命名の趣味、より良い書き方の提案はコメントとして残してよいが、それを理由に
不合格にしてはならない。

この線引きが必要なのは、コードレビューには動作確認のような客観的な失敗条件が無く、
LLM がいくらでも指摘を作れてしまうためである。**差し戻しの根拠をリポジトリに書かれた
基準に接地させる**ことで、エージェントが確定的に直せる指摘だけが差し戻しループに乗る。

#### 検証手順はリポジトリ側に置く

「何をもって合格とするか」はリポジトリごとに異なる。これを Go 側の設定で持つと対象
リポジトリが増えるたびに autopilot の変更が必要になり、3.3 節に反する。そこで
**リポジトリ直下の `.agents/verify.md` に検証手順（プレビュー環境の URL、実行してよい検証
コマンド、合格条件、対象外とするもの）を書き、verify のプロンプトはそれを読ませるだけ**とする。
`AGENTS.md` を読ませているのと同じ流儀であり、Go 側の変更なしにリポジトリを増やせる。
...
`verify_inconclusive` は `verify_passed` と同じく `ready` に進める。**verify が判定を出せない
ときに PR が詰まる**という最大のリスクを消すためである。プロンプト側でも「判断がつかない
場合は不合格にしてはならない」を明示する。LLM は迷うと「念のため不合格」へ倒れやすく、
それが偽陽性の主因になる。

同じ理由から、**verify 自体を起動できなかった場合も `blocked` にせず `ready` へ進める**
（実装エージェント側の同種の失敗が `blocked` になるのとは異なる）。作業ディレクトリの用意に
失敗した程度で PR を止めるのは「判定が無い」を「不合格」に読み替えることに等しい。
検証されないまま進んだ事実は PR コメントと Error ログに残す。
...
実装と verify の差はプロンプトの中身だけであり、`internal/runner` はモードを知らない汎用の
CLI 起動役に留まる。

verify のために新しい phase は追加しない。`in_review` と `ready` で表現でき、`verify_failure` は
`events.type` に入る文字列にすぎない。判定を永続化しないのは **GitHub が真実である**ため
（3.2 節）。
